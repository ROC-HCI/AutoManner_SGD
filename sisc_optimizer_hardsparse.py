...
def shrink(alpha, threshold):
    ...
    return alpha
# Project into a positive and sparse version to enforce l-0 norm
def sparsify(alpha,max_l0):
    alpha[alpha<0]=0 # Project alpha into {x:x>=0}
    for d in xrange(np.size(alpha,axis=1)):
        mask = np.zeros(len(alpha))
        temp = [[item,idx] for idx,item in enumerate(alpha[:,d])]
        heapify(temp)
        idx = np.array(nlargest(max_l0,temp),dtype=int)[:,1]
        mask[idx]=1.0
        alpha[:,d]*=mask
    return alpha
...
def optimize_proxim(X,M,D,beta,iter_thresh=65536,\
    ...
    while iter < iter_thresh:
        ...
        for trialno in xrange(int(math.ceil(math.log(initLR,2)))+5):
            ...
            newAlpha = sparsify(alpha - gamma_alpha*gralpha/N,10)
...
#    #psi = psi_orig                 # Setting the initial value to actual
#    #alpha = alpha_orig             # solution. Debug purpose only
#    countZero,setZero_a = 0,True,True
#    previtcost = logcost(X,alpha,psi,beta,workers)
#    # Main optimization loop
#    while iter < iter_thresh:
#        itStartTime = time.time()
